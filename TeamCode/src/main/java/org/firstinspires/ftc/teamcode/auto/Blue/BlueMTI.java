...
@Autonomous(name = "BlueMTI", group = "Concept")
public class BlueMTI extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        if (position == BlueSpikemarkPipeline.SpikemarkPosition.ONE || position == BlueSpikemarkPipeline.SpikemarkPosition.TWO || position == BlueSpikemarkPipeline.SpikemarkPosition.THREE) {
            drive.followTrajectorySequenceAsync(drive.trajectorySequenceBuilder(startPose)
                    // for all
                    .lineToSplineHeading(new Pose2d(startPose.getX() + (0.5 * TILE_SIZE) + 23, startPose.getY()-(1.2 * TILE_SIZE), Math.toRadians(180)))
                    .turn(0.2)
                    .waitSeconds(1)
                    ...
                    .UNSTABLE_addTemporalMarkerOffset(0, () -> {
                        ...
                        in_out_take.setSpecificInstruction(IntakeOuttake.SpecificInstructions.EXTEND_HORIZONTAL);
                    })
                    .waitSeconds(1.5)
                    .UNSTABLE_addTemporalMarkerOffset(0, () -> {
                        Intake.outtake();
                    ...
                    .waitSeconds(0.2)
                    .forward(0.1*TILE_SIZE)
                    .waitSeconds(1)
                    .lineToSplineHeading(new Pose2d(startPose.getX() + 23, startPose.getY()-(1.1 * TILE_SIZE), Math.toRadians(180)))
                    .lineToSplineHeading(new Pose2d(startPose.getX() - (2.3 * TILE_SIZE) + 23, startPose.getY()-(1.1 * TILE_SIZE), Math.toRadians(180)))
                    .waitSeconds(3)
                    .UNSTABLE_addTemporalMarkerOffset(0, () -> {
                        in_out_take.setInstructions(IntakeOuttake.Instructions.OPEN_INTAKE);
                        in_out_take.setSpecificInstruction(IntakeOuttake.SpecificInstructions.MAXEXTEND_VERTICAL);
                    })
                    .waitSeconds(5)
                    //.strafeRight((0.4 * TILE_SIZE))
                    //.forward(20) //delete this later
                    //cycle 1
